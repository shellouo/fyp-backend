...
// Animate meshes & particles
  meshes.forEach((mesh, i) => {
    // 1. Animate mesh rotation（所有支持 rotation 的对象）
    if (mesh instanceof THREE.Object3D && mesh.rotation) {
      const speed = params.value.rotationSpeed;
      mesh.rotation.y += delta * speed * (i % 2 ? -1 : 1) * 0.3;
      mesh.rotation.x += delta * speed * 0.1;
    }

    // 2. Animate particles (only for Points)
    if (
...
function cleanup() {
  ...
  meshes.forEach((mesh) => {
    // 只对有 geometry 的对象（Mesh/Points/Line）做处理
    if (
        (mesh instanceof THREE.Mesh || mesh instanceof THREE.Points || mesh instanceof THREE.Line) &&
        mesh.geometry
    ) {
